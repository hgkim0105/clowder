...
생성된 DMG를 열어 Clowder.app을 Applications 폴더로 드래그한 뒤 실행하세요. 이미 실행 중인 Claude Code 세션은 훅을 다시 로드하기 위해 재시작해야 합니다.

`install-hooks.sh`는 idempotent — 다시 실행해도 중복 등록되지 않습니다. 수동 등록을 원하면 [Claude Code 훅 연동](#claude-code-훅-연동)을 참고하세요.
